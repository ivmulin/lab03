...
int main() {
  ...
  if (!csv_file.is_open()) {
    std::cerr << "Error: Could not open " << result_filename
              << " for writing!\n";
    return 1;
  }

  // Обновленный заголовок со всеми структурами
  ...
  // Определение лямбды для SqrtDecomposition (минимум на отрезке)
  ...
  for (int step = 0; step < k; ++step) {
    ...
    double rsq1d_query_avg =
        static_cast<double>(rsq1d_query_ops_sum) / num_queries;
    ...
    for (int q = 0; q < num_queries; ++q) {
      int r1 = idx_dist(rng), c1 = idx_dist(rng), r2 = idx_dist(rng),
          c2 = idx_dist(rng);
      if (r1 > r2)
        std::swap(r1, r2);
      if (c1 > c2)
    ...
    double rsq2d_query_avg =
        static_cast<double>(rsq2d_query_ops_sum) / num_queries;

    // --- RMQ 1D (Precalculated) ---
    RMQ1DPrecalc rmq1d;
    ...
    double rmq1d_query_avg =
        static_cast<double>(rmq1d_query_ops_sum) / num_queries;
    ...
    long long sqrt_query_ops_sum = 0;
    long long sqrt_update_ops_sum = 0;
    for (int q = 0; q < num_queries; ++q) {
      int l = idx_dist(rng), r = idx_dist(rng);
      if (l > r)
        std::swap(l, r);
      ...
      sqrt_update_ops_sum += sqrt_decomp.update(idx, val);
    }
    double sqrt_query_avg =
        static_cast<double>(sqrt_query_ops_sum) / num_queries;
    double sqrt_update_avg =
        static_cast<double>(sqrt_update_ops_sum) / num_queries;

    // --- Дерево отрезков (RMQ) ---
    SegmentTree seg_tree;
    long long seg_build_ops = seg_tree.build(arr_1d);
    long long seg_query_ops_sum = 0;
    long long seg_update_ops_sum = 0;
    for (int q = 0; q < num_queries; ++q) {
      int l = idx_dist(rng), r = idx_dist(rng);
      if (l > r)
        std::swap(l, r);
      ...
      int idx = idx_dist(rng);
      int val = val_dist(rng);
      seg_update_ops_sum += seg_tree.update(idx, val);
    }
    double seg_query_avg = static_cast<double>(seg_query_ops_sum) / num_queries;
    double seg_update_avg =
        static_cast<double>(seg_update_ops_sum) / num_queries;

    // Запись строки в CSV
    csv_file << n << "," << rsq1d_build_ops << "," << rsq1d_query_avg << ","
             << rsq2d_build_ops << "," << rsq2d_query_avg << ","
             << rmq1d_build_ops << "," << rmq1d_query_avg << ","
             << sqrt_build_ops << "," << sqrt_query_avg << ","
             << sqrt_update_avg << "," << seg_build_ops << "," << seg_query_avg
             << "," << seg_update_avg << "\n";
  }
